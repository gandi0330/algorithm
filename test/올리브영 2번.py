...
def solution(subway, start, end):
    def recur (start_s, middle_s, end):
        if end in start_s:
            return 1
        else :
            for idx in range(len(middle_s)):
                if middle_s[idx] in start_s:
                    
                    if idx == len(middle_s):
                        return 1 + recur(middle_s[idx], middle_s[:idx], end)
                    else:
                        return 1 + recur(middle_s[idx], middle_s[:idx]+middle_s[idx+1:], end)
                        
                
    i = 0
    for idx in range(len(subway)):
        subway[idx] = [*map(int, subway[idx].split())]
        if start in subway[idx]:
            start_station = subway[idx]
            i = idx
    del subway[i]
    return recur(start_station, subway, end)
